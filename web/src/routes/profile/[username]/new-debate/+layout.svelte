<script lang="ts">
	...
	onMount(async () => {
		...
		if( typeof storedLiveDebateId === "string" && storedLiveDebateId.includes("-") ) {
			const { data, error } = await supabase.from("live_debate").select().eq("id", storedLiveDebateId).single();
			$liveDebate = data;
			if( error ) {
	...
	async function createLiveDebate(): Promise<Tables<"live_debate"> | null > {
		...
		const { data: newLiveDebate, error: newLiveDebateError } = await supabase.from("live_debate").insert({
			...
			const { data: liveDebateRole, error: liveDebateError } = await supabase.from("live_debate_roles").insert({
				role: "host",
