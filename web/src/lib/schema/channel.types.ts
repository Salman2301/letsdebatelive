...
export const channelPayload: Record<(typeof channelList)[number], InfoAndPayload> = {
	'host:video_activity:$sec_channel_id': {
		description:
			'Host and the Video feed setting used between control and record room, Consumed by ghost user',
		trigger: 'host',
		role: {
			write: ['host'],
	...
	'host:setting_activity:$channel_id': {
		description:
			'Host can change the setting live support only, follower only and anyone can consume it',
		trigger: 'host',
		role: {
			write: ['host'],
			read: ['user']
	...
	'service:notification_read:$channel_id': {
		description:
			'Only write by one of the service used to check if the notification is read, only host can see',
		trigger: 'service',
		role: {
			write: ['service'],
			read: ['host']
	...
	'backstage:activity:$channel_id': {
		description:
			'Used by the backstage and the host. User might have access since we need to see the raise hands',
		trigger: 'backstage',
		role: {
			write: ['backstage'],
			read: ['user']
	...
	'backstage:chat:$channel_id': {
		description:
			'Used by the backstage and the host. Communication between only host and the backstage',
		trigger: 'backstage',
		role: {
			write: ['backstage'],
			read: ['moderator']
